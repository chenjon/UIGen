...
vi.mock("../ToolCallIndicator", () => ({
  ToolCallIndicator: ({ toolInvocation }: { toolInvocation: { toolName: string } }) => (
    <div data-testid="tool-call-indicator">{toolInvocation.toolName}</div>
  ),
}));
...
test("MessageList renders messages with parts", () => {
  ...
  expect(screen.getByTestId("tool-call-indicator")).toBeDefined();
});
